...
QuickClipper を OBS と連携させるには、
Settings タブに OBS WebSocket の接続情報（ポート番号・パスワード）を入力する必要があります。

- OBS 側で WebSocket が有効化されていること
...
接続情報が正しく設定されていないと、
クリップ保存やリプレイバッファ制御が正常に行えません。


## **OBS のリプレイバッファ最大時間の設定**
...
- パフォーマンス優先で **120〜300 秒程度に短く設定**することも可能です。
  その場合、プリセットは **指定時間ではなく OBS の最大時間に自動短縮**されます。
...
### ■ クリップの長さがおかしい
QuickClipper が保存できる長さは、
OBS に設定されているリプレイバッファの最大時間を上限として調整されます。
